...
import numpy as np
import plotly.express as px  # ← FOR YOUR PLOTS
import matplotlib.pyplot as plt  # ← FOR YOUR PLOTS
...
if search:
    ...
    if not result.empty:
        ...
        st.error("Element not found. Try 'Hydrogen', 'H', or '1'")

# --- PROPERTY SELECTOR ---
prop = st.sidebar.selectbox("Color by:", ['Density', 'Electronegativity', 'Melting_Point'])

# --- FIXED PERIOD & GROUP ---
def get_period(atomic):
    if atomic <= 2: return 1
    elif atomic <= 10: return 2
    elif atomic <= 18: return 3
    elif atomic <= 36: return 4
    elif atomic <= 54: return 5
    elif atomic <= 86: return 6
    else: return 7

def get_group(atomic):
    return (atomic - 1) % 18 + 1

plot_df = df.copy()
plot_df['Period'] = plot_df['Atomic_Number'].apply(get_period)
plot_df['Group'] = plot_df['Atomic_Number'].apply(get_group)

# --- FIXED HEATMAP: FULL GRID, NO OVERLAP ---
# Create full 7x18 grid
full_grid = pd.DataFrame(
    index=range(1, 8),  # Periods
    columns=range(1, 19)  # Groups
)

# Fill with values
for _, row in plot_df.iterrows():
    full_grid.loc[row['Period'], row['Group']] = row[prop]

fig, ax = plt.subplots(figsize=(18, 8))
sns.heatmap(
    full_grid, annot=True, fmt=".1f", cmap="viridis",
    cbar_kws={'label': prop}, ax=ax, linewidths=0.5, linecolor='gray'
)
ax.set_title(f"Periodic Table: {prop} (Fixed Grid)", fontsize=16, pad=20)
ax.set_xlabel("Group")
ax.set_ylabel("Period")
st.pyplot(fig)

# --- YOUR INTERACTIVE PLOT ---
st.write("**Density vs Atomic Number (Interactive)**")
density_df = df[['Atomic_Number', 'Density', 'Phase', 'Name']].dropna()
fig_px = px.scatter(
...
# --- FOOTER ---
st.caption("Built by baby chemist 🍼 | Fixed heatmap + interactive Plotly")
