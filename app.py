...
# === 7. RADIOACTIVITY DISTRIBUTION ACROSS PERIODIC BLOCKS ===
st.subheader("7. Radioactivity Across Periodic Blocks (s = chill, f = apocalypse)")

block_radio = df.groupby(['Block', 'Radioactivity']).size().reset_index(name='Count')

# Keep your consistent badass color scheme
radio_color_map = {
    'No': '#00CCFF',         # safe & chill
    'Primordial': '#00FF99', # the two green loners (Th, U)
    'Yes': '#FF3333'         # red = run
}
...
fig7.update_layout(
    ...
    legend_title_text="Radioactivity Level",
    title=dict(
        text="<b>Radioactivity by Periodic Block</b><br><sup>s & p = basically safe • d = mostly safe • f = radioactive hellscape</sup>",
        font=dict(size=20),
        x=0.5, xanchor="center"
    ),
    xaxis_title="Periodic Block",
