...
if search:
    ...
    if 'Symbol' in df.columns and 'Name' in df.columns:
        ...
        result = df[df['Atomic Number'].astype(str).str.contains(search)]
...
for _, r in plot_df.iterrows():
    ...
    try:
        ...
        full_grid.loc[r['Period'], 'Group'] = np.nan
...
density_df = df[['Name', 'Atomic Number', 'Density', 'Phase']].dropna(subset=['Density']).copy() # Changed column order
fig_px = px.scatter(
    density_df, x='Atomic Number', y='Density', color='Phase',
    hover_name='Name', # Changed hover_name to 'Name'
